...
class SimpleConfig(PrintError):
    ...
    def __init__(self, options=None, read_user_config_function=None,
        ...
        self.user_config = {}  # for self.get in electrumbth_path()
    ...
    def electrum_path(self):
        ...
        path = self.get('electrumbth_path')
        if path is None:
            path = self.user_dir()
        ...
        self.print_error("electrumbth directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(
                _('ElectrumBTH datadir does not exist. Was it deleted while running?') + '\n' +
